  ...
  <div>
    ...
    <a-form-model-item ref="region" prop="region" v-if="form.country !== 'Страна'">
      <a-select @change="changeSelectedRegion" placeholder="Регион" style="width: 190px" v-model="form.region">
        <a-select-option v-for="region in selectedCountry.areas" :key="region.id">
    ...
    <a-form-model-item ref="city" prop="city" v-if="form.region !== 'Регион' && selectedRegion.areas.length > 0">
      <a-select placeholder="Город" style="width: 190px" v-model="form.city">
        <a-select-option v-for="city in selectedRegion.areas" :key="city.id">
...
<script>
import formValidator from  '../assets/js/form-validator';
...
import { mapActions, mapGetters } from 'vuex'
  ...
  export default {
    ...
    methods: { 
      ...
      changeSelectedCountry(selectedCountryID){
        this.selectedCountry = this.countries.find(country => country.id === selectedCountryID)
        this.form.region = 'Регион'
      },
      changeSelectedRegion(selectedRegionID){
        this.selectedRegion = this.selectedCountry.areas.find(region => region.id === selectedRegionID)
        this.form.city = 'Город'
      },
      onSubmit() {
        this.$refs.rules.validate(valid => valid? submitForm(this.form) : false)
      },
    },
...
@import '../assets/styles/main-form.less';
</style>